  ...
  return (
    <AuthLayout>
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard label="Pending Jobs" value={pendingJobs} to="/jobs" color="yellow" />
        <StatCard label="Approved Jobs" value={approvedJobs} to="/jobs" color="green" />
        <StatCard label="Rejected Jobs" value={rejectedJobs} to="/jobs" color="red" />
        <StatCard label="Pending Approvals" value={pending?.length ?? 0} to="/pending" color="blue" />
      </div>

      <div className="mt-12">
        <h2 className="mb-4 text-lg font-semibold text-gray-900">Quick Actions</h2>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <ActionCard title="Moderate Jobs" description="Review and approve or reject job listings" to="/jobs" />
          <ActionCard title="Pending Approvals" description={`${pending?.length ?? 0} employer${(pending?.length ?? 0) !== 1 ? 's' : ''} awaiting verification`} to="/pending" />
          {isSuperAdmin && (
            <>
              <ActionCard title="Manage Employers" description="View and manage verified employers" to="/employers" />
              <ActionCard title="Manage Admins" description={`${admins?.length ?? 0} admin account${(admins?.length ?? 0) !== 1 ? 's' : ''} configured`} to="/admins" />
            </>
          )}
        </div>
      </div>
