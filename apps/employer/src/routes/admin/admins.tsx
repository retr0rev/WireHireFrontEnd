...
function AdminAdminsPage() {
  ...
  return (
    <AuthLayout>
      ...
      {showForm && (
        <form onSubmit={handleCreate} className="mb-6 max-w-md rounded-xl border border-gray-200 bg-white p-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <select value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value as AdminRole })} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-gray-500 focus:outline-none">
                <option value="moderator">Moderator</option>
                <option value="super_admin">Super Admin</option>
              </select>
            </div>
          </div>
          <button type="submit" disabled={createAdmin.isPending} className="mt-4 rounded-lg bg-gray-900 px-4 py-2 text-sm font-medium text-white hover:bg-gray-800 disabled:opacity-50">
            ...
            {admins.map((admin) => (
              <div key={admin.id} className="rounded-xl border border-gray-200 bg-white p-4">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <span className="shrink-0 rounded-full bg-gray-100 px-2 py-0.5 text-xs text-gray-600">{admin.admin_role}</span>
                </div>
              </div>
            ))}
          </div>
                ...
                {admins.map((admin) => (
                  <tr key={admin.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3">
                      <span className="rounded-full bg-gray-100 px-2 py-0.5 text-xs text-gray-600">{admin.admin_role}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
